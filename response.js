...
ThreadManager.i[room] = new java.lang.Thread(new java.lang.Runnable(){ run : function(){
try { java.lang.Thread.sleep(30000);
replier.reply(String.fromCharCode(0) + '타임어택 종료!');
temp.hanQuizValid[room] = false; } catch (e) {}
} });
...
function miscSys(params) {
   ...
   if (msg.substr(0, c.length+1) == c + ' ') {
   	replier.reply(lolStat(msg.substring(5)));
	}}
   loop: {
   c = '!실검'
   a = 'all'
